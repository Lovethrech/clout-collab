...
const toggleOption = (field, option) => {
  const currentValues = form.value[field]

  if (currentValues.includes(option)) {
    form.value[field] = currentValues.filter((item) => item !== option)
  } else {
    form.value[field] = [...currentValues, option]
  }
}
...
const getSelectedText = (field, placeholder) => {
  ...
  return `${form.value[field].length} selected`
}
